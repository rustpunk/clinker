...
use crate::config::composition::{
    LayerKind, ResourceBinding, SchemaLayer, SchemaProvRecorder, SchemaProvenanceDb,
...
pub enum OverlayOp {
    ...
    /// Rebind one declared composition resource slot to a logical catalog id.
    /// Constructed from the strict top-level `resources:` overlay surface;
    /// never deserialized through the structural `overrides:` grammar.
    BindResource(BindResourceOp),
}

/// Typed resource-binding clobber generated by channel/group resolution.
#[derive(Debug, Clone)]
pub struct BindResourceOp {
    /// Composition node name in the current pipeline scope.
    pub target: String,
    /// Public slot declared by `_compose.resources_schema`.
    pub slot: String,
    /// Secret-free logical workspace-catalog identity.
    pub binding: crate::resources::LogicalResourceId,
    /// Exact semantic provenance layer, including group sequence.
    pub provenance_layer: LayerKind,
    /// Whether this layer locks the binding against higher layers.
    pub fixed: bool,
}
...
pub enum OverlayOpError {
    ...
    /// A resource overlay targeted a non-composition node.
    ResourceTargetNotComposition {
        target: String,
        actual: &'static str,
        span: Span,
    },
    /// A resource overlay attempted to select credentials.
    CredentialSelector { slot: String, span: Span },
}
...
impl OverlayOpError {
    ...
    pub fn span(&self) -> Span {
        match self {
            ...
            | OverlayOpError::BypassInvalid { span, .. }
            | OverlayOpError::ResourceTargetNotComposition { span, .. }
            | OverlayOpError::CredentialSelector { span, .. } => *span,
        }
    }
}
...
impl std::fmt::Display for OverlayOpError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            OverlayOpError::ResourceTargetNotComposition {
                target,
                actual,
                span,
            } => write!(
                f,
                "resource binding targets node {target:?}, which is a {actual} node rather than a composition{}",
                at(*span)
            ),
            OverlayOpError::CredentialSelector { slot, span } => write!(
                f,
                "resource binding slot {slot:?} is a credential selector; remove it and select credentials only through explicit run preflight{}",
                at(*span)
            ),
        }
    }
...
pub fn apply_overlay_ops_recording(
    ...
    for layered in ops {
        ...
        match layered.op.value {
            ...
            OverlayOp::BindResource(binding) => apply_bind_resource(&mut nodes, binding, span)?,
        }
    }
...
fn apply_bind_resource(
    nodes: &mut [Spanned<PipelineNode>],
    op: BindResourceOp,
    span: Span,
) -> Result<(), OverlayOpError> {
    if is_reserved_credential_selector(&op.slot) {
        return Err(OverlayOpError::CredentialSelector {
            slot: op.slot,
            span,
        });
    }
    let Some(index) = index_of(nodes, &op.target) else {
        return Err(OverlayOpError::MissingTarget {
            op: "resource binding",
            target: op.target,
            span,
        });
    };
    let actual = nodes[index].value.type_tag();
    let PipelineNode::Composition { resources, .. } = &mut nodes[index].value else {
        return Err(OverlayOpError::ResourceTargetNotComposition {
            target: op.target,
            actual,
            span,
        });
    ...
    resources
        .entry(op.slot)
        .and_modify(|current| {
            current.apply_layer(op.binding.clone(), op.provenance_layer, span, op.fixed);
        })
        .or_insert_with(|| {
            ResourceBinding::from_layer(op.binding, op.provenance_layer, span, op.fixed)
        });
    Ok(())
}
