...
/// - both `IEJoin` (pure-range) and `HashPartitionIEJoin` (equi+range), which
///   share the one block-band path: each side is external-sorted and the merged
///   stream is re-sliced into per-block spill files (equality, when present, is
///   just an added prune axis inside the same machinery),
...
pub(super) fn writes_spill_files(node: &PlanNode) -> bool {
    ...
    match node {
        ...
        // Both IEJoin (pure-range) and HashPartitionIEJoin (equi+range) run the
        // one bounded block-band path: it external-sorts each side into runs and
        // re-slices the merged stream into per-block spill files, so both write
        // real spill files and must appear in the disk-spill projection surfaces.
...
///
/// Most spilling operators write their accumulated input once. The block-band
/// `IEJoin` / `HashPartitionIEJoin` path writes it about twice: it
/// external-sorts each side into runs (pass one), then re-slices the merged
/// stream into per-block spill files (pass two). Returns `1` for a node that
/// does not write spill files.
pub(super) fn spill_volume_multiplier(node: &PlanNode) -> u64 {
    use crate::plan::combine::CombineStrategy;
    match node {
        PlanNode::Combine {
            strategy: CombineStrategy::IEJoin | CombineStrategy::HashPartitionIEJoin { .. },
            ..
        } => 2,
        _ => 1,
    }
