...
pub struct BoundBody {
    ...
    /// Per-input-port: the upstream node name in the parent scope
    /// that supplies records for the port. Order matches the
    /// signature's `inputs:` declaration. The body executor reads
    /// this at composition entry to collect parent-scope records by
    /// port name before entering the body's own DAG walk.
