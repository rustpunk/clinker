...
/// to: the `_cxl_dlq_id`, `_cxl_dlq_failure_id` and `_cxl_dlq_timestamp` of
/// the row it becomes.
///
...
///
/// The failure id is the `_cxl_dlq_id` of the trigger row of the failure
/// that produced this row, so every row one failure produced shares it. A
/// trigger's failure id is its own id ([`Self::now`]). A second row of the
...
impl DlqFailureStamp {
    ...
    /// own failure, so its failure id is its id.
    pub fn now() -> Self {
    ...
    /// to: the row's `_cxl_dlq_failure_id`.
    pub fn failure_id(&self) -> uuid::Uuid {
        self.failure
    }
    ...
    /// failure: the same time and failure id under a fresh id, because every
    /// row's id is unique.
    ...
    /// `cause`'s failure id, because that failure took the row with it.
    ...
    /// failure id is `id`. Its only caller, the source ordering barrier,
...
pub struct DlqEntry {
    ...
    /// condemned it. Serialized as `_cxl_dlq_id`, `_cxl_dlq_failure_id` and
    /// `_cxl_dlq_timestamp`.
...
mod tests {
    ...
    fn stamp_now_is_its_own_failure() {
        ...
        assert_eq!(stamp.failure_id(), stamp.id());
    ...
    fn sibling_keeps_the_failure_under_a_new_id() {
        ...
        assert_eq!(sibling.failure_id(), trigger.id());
        assert_eq!(sibling.at(), trigger.at(), "the same observed failure");
        assert_eq!(
            sibling.sibling().failure_id(),
            trigger.id(),
    ...
    fn condemned_by_takes_the_cause_failure_with_its_own_id_and_time() {
        ...
        assert_eq!(collateral.failure_id(), trigger.id());

        let build = trigger.sibling();
        assert_eq!(
            DlqFailureStamp::condemned_by(&build).failure_id(),
            trigger.id(),
    ...
    fn from_parts_rebuilds_a_trigger_stamp() {
        ...
        assert_eq!(rebuilt.failure_id(), rebuilt.id());
    }
}
