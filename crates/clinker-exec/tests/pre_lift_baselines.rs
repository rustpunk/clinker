...
struct CompositionFixture {
    /// Stable identifier.
    name: &'static str,
    /// Path relative to `crates/clinker-exec/tests/fixtures/`.
    rel_path: &'static str,
...
fn snapshot_composition_pipeline(snap_name: &str, rel_path: &str) {
    ...
    let (dag, _) =
        clinker_exec::executor::PipelineExecutor::explain_plan_dag(&compiled).expect("explain_dag");
    let text = dag.explain_text(&config);
...
fn test_insta_baselines_are_tail_var_stable() {
    ...
    for _ in 0..2 {
        ...
        let compiled =
            clinker_plan::config::PipelineConfig::compile(&config, &ctx).expect("compile");
        let (dag, _) = clinker_exec::executor::PipelineExecutor::explain_plan_dag(&compiled)
            .expect("explain_dag");
        let text = dag.explain_text(&config);
