...
pub struct Group {
    ...
    /// block uses (`static` / `pipeline` / `source` / `record`).
    pub vars: GroupVars,
...
/// The four-scope `vars:` surface shared by pipelines, channels, and groups.
///
/// Each scope maps a flat var name to its [`ScopedVarDecl`] (`{ type, default }`),
/// matching the pipeline `vars:` declaration shape. Overriding an existing
/// declaration matches its type; adding a new one becomes the declaration.
#[derive(Debug, Clone, Default, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct GroupVars {
    /// Static config vars, read via `$vars.<key>` (YAML key `static`).
    #[serde(default, rename = "static")]
    pub static_block: IndexMap<String, ScopedVarDecl>,
    /// Pipeline-scope vars, read via `$pipeline.<key>`.
    #[serde(default)]
    pub pipeline: IndexMap<String, ScopedVarDecl>,
    /// Source-scope vars, read via `$source.<key>`. Outer key is the
    /// source-node name; inner key is the var name.
    #[serde(default)]
    pub source: IndexMap<String, IndexMap<String, ScopedVarDecl>>,
    /// Record-scope vars, read via `$record.<key>`.
    #[serde(default)]
