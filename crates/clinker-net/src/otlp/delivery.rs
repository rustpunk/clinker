//! The OTLP delivery loop itself: request construction, the finite retry
//! budget, and the classification of what a failed exchange proved.
//!
//! Split from the endpoint admission in the parent module because only this
//! half needs an HTTP client. Admission decides what a transport is allowed to
//! reach and is a property of the configuration, so it stays compiled when the
//! `transport` feature is off; nothing here can be.
...
use super::{
    ...
    OtlpCredentialRequest, OtlpDeliveryBudget, OtlpDeliveryFailure, OtlpDeliveryFailureKind,
    OtlpDeliveryOutcome, OtlpRetryCause, OtlpSignal,
};
    ...
    fn new(signal: OtlpSignal, kind: OtlpDeliveryFailureKind, attempts: u32) -> Self {
        ...
        Self {
            ...
            classification: code.map(|code| {
                clinker_core_types::FailureClassification::for_code(code)
...
impl AdmittedOtlpEndpoint {
    fn route(&self, signal: OtlpSignal) -> Result<Uri, http::Error> {
        Uri::builder()
            .scheme(
                self.origin
                    .scheme()
...
fn retry_after_hint(headers: &http::HeaderMap) -> Option<RetryAfterHint> {
    let mut longest: Option<Duration> = None;
    for value in headers.get_all(http::header::RETRY_AFTER) {
...
mod classification {
    ...
    fn retry_after(values: &[&str]) -> http::HeaderMap {
