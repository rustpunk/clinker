...
#[cfg(feature = "lineage")]
use lifecycle::{RunLifecycleSnapshot, RunLifecycleStartFacts};
...
// Raised when a bounded delivery worker cannot be started or cannot finish.
// Both such workers — the OTLP exporter and the lineage exporter — are
// optional capabilities, so a build with neither has nothing to raise it.
...
fn run(args: &RunArgs, machine: Option<&MachineEmitter>) -> Result<u8, PipelineError> {
    ...
    capability::check_observability(&observability_policy)?;
    #[cfg(feature = "otlp")]
    let mut otlp_runtime = observability::OtlpRuntimeBundle::admit(&observability_policy)
        .map_err(observability_configuration_error)?;
    // Every statement below that names `clinker_lineage` is gated the same way
    // and for the same reason: `capability::check_lineage_requested` has
    // already refused `--lineage` and `--lineage-events` in a build without the
    // feature, so in that build these conditions are all statically false.
    #[cfg(feature = "lineage")]
    ...
    // Before the plan compiles, before any source is discovered, and before a
    // byte is staged: a construct this build cannot carry out is refused while
    // there is still nothing to unpick.
    capability::check_pipeline(&pipeline_config)?;
    capability::check_lineage_requested(args.lineage.is_some() || args.lineage_events.is_some())?;
    ...
    // Nothing would drain the arena, so nothing reserves one. The executor
    // then records no telemetry at all, which is what the refusal in
    // `capability::check_observability` tells an author to expect.
    #[cfg(not(feature = "otlp"))]
    let telemetry_handles: Option<(
    ...
    #[cfg(not(feature = "otlp"))]
    let (telemetry_producer, mut otlp_worker): (
        Option<clinker_exec::telemetry::TelemetryProducer>,
        Option<observability::OtlpWorker>,
    ) = {
        drop(telemetry_handles);
        drop(telemetry_correlation);
        (None, None)
    };
    ...
    if let Some(path) = &args.lineage_events {
        ...
        if let Some(delivery_config) = configuration.delivery {
            ...
            lineage_output = Some(LiveLineageOutput::new(sink, lineage, job));
        } else {
            ...
            let writer: Box<dyn std::io::Write> = if path.as_os_str() == std::ffi::OsStr::new("-") {
                ...
                match std::fs::metadata(path) {
                    ...
                    Ok(metadata) if !metadata.is_file() && !metadata.is_dir() => {
                        Box::new(LazyLineageFile::deferred(path.clone()))
                    }
                    _ => Box::new(
                        std::fs::File::create(path)
            ...
            lineage_output = Some(LiveLineageOutput::new(
    ...
    for body in pipeline_config.source_bodies() {
        ...
        match &source.transport {
            ...
            // A build without the `rest` capability never reaches this arm:
            // `capability::check_pipeline` refused the same pipeline before the
            // plan compiled, so this is the only place the transport is named.
            #[cfg(feature = "rest")]
            ...
            clinker_plan::config::SourceTransport::Rest(_) => {
                return Err(PipelineError::Internal {
                    op: "rest source discovery",
                    node: source.name.clone(),
                    detail: "a rest source survived the compiled-out capability check".to_owned(),
                });
            }
        }
    }
