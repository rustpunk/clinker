...
fn insert_field(nodes: &mut Vec<FieldNode>, path: &str, text: &str) {
    if let Some((first, rest)) = path.split_once('.') {
        ...
        let branch = nodes.iter_mut().find(|n| matches!(n, FieldNode::Branch { name, .. } if name == first));
        if let Some(FieldNode::Branch { children, .. }) = branch {
            insert_field(children, rest, text);
        } else {
            let mut children = Vec::new();
            insert_field(&mut children, rest, text);
            nodes.push(FieldNode::Branch { name: first.to_string(), children });
        }
    } else {
        nodes.push(FieldNode::Leaf { name: path.to_string(), text: text.to_string() });
    }
}
...
mod tests {
    ...
    fn test_xml_write_basic_structure() {
        ...
        let records = vec![make_record(&schema, "Alice", 30), make_record(&schema, "Bob", 25)];
    ...
    fn test_xml_write_nested_expansion() {
        ...
        assert!(output.contains("<Address><City>NYC</City></Address>"),
    ...
    fn test_xml_write_shared_prefix_grouping() {
        ...
        assert_eq!(output.matches("<Address>").count(), 1,
    ...
    fn test_xml_write_preserve_nulls_true() {
        ...
        let config = XmlWriterConfig { preserve_nulls: true, ..Default::default() };
        let output = write_records(config, &[record], &schema);
        assert!(output.contains("<b/>"), "Null field should be self-closing: {output}");
    }
    ...
    fn test_xml_write_preserve_nulls_false() {
        ...
        let config = XmlWriterConfig { preserve_nulls: false, ..Default::default() };
        let output = write_records(config, &[record], &schema);
        assert!(!output.contains("<b"), "Null field should be omitted: {output}");
    }
    ...
    fn test_xml_write_escaping() {
        ...
        let record = Record::new(Arc::clone(&schema), vec![
            Value::String("a & b < c > d \"e\" 'f'".into()),
        ]);
        let output = write_records(XmlWriterConfig::default(), &[record], &schema);
        assert!(output.contains("&amp;"), "& should be escaped: {output}");
        assert!(output.contains("&lt;"), "< should be escaped: {output}");
        assert!(output.contains("&gt;"), "> should be escaped: {output}");
        assert!(!output.contains("a & b"), "Raw & should not appear: {output}");
    }
    ...
    fn test_xml_roundtrip_reader_writer() {
        ...
        let records = vec![
            Record::new(Arc::clone(&schema), vec![
                Value::String("Alice".into()), Value::Integer(42),
            ]),
            Record::new(Arc::clone(&schema), vec![
                Value::String("Bob".into()), Value::Integer(99),
            ]),
        ];

        // Write
        let output = write_records(
            XmlWriterConfig { preserve_nulls: true, ..Default::default() },
            &records,
            &schema,
        );
