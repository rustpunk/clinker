...
- Its native GitHub `blocked by` relationships are empty, closed, merged, or
  explicitly waived; fallback body/comment blockers are also resolved.
...
- Use `Closes #...` only for the exact implementation issue(s), not a parent
  milestone or epic.
