...
Use GitHub's native issue relationships as the primary dependency signal:

- On the dependent issue, mark prerequisite issues or PRs as `blocked by`.
- GitHub will show dependent issues as `blocking` on the prerequisite.
- Use body text or comments only as a fallback/mirror for external blockers,
  credentials, unavailable tool permissions, or human-readable context.

If native relationships are unavailable, mirror the dependency with:
