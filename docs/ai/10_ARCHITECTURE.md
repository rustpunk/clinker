...
11. Dead-lettered rows stream at failure time through the caller-supplied `DlqSink` (`WriterRegistry.dlq_sink`), each encoded under its bucket's plan-time header; the CLI supplies `StagedDlqSink`, which writes staged files published with the run. A dead letter with a destination and no sink fails the run. `ExecutionReport` returns counters, DLQ counts (per source, and per stage, category and bucket in `DlqReport`), execution summary, peak RSS, CPU/IO totals, stage metrics, watermarks, rollback cursors, per-source counts, spill totals, streaming charge peaks, and interrupted status.
...
| Source typing, fan-out, and ordering | `CompiledSourceOrder`, typed key positions/types, per-file scope, unsorted policy, and the typed `max_output_rows_per_input` source config | Lazy JSON/XML mixed-radix cursors, structured N+1 source rejection, unified attempt stream, population accounting, and `SourceFileOrderBarrier` | Fan-out retains no output product; records and recoverable failures share one attempt order; separate files never imply a global sort |
