...
- **One oversized correlation group trips `E310` in [Reshape](../nodes/reshape.md#limits) and [Cull](../nodes/cull.md#limit-a-single-group-must-fit-the-finalize-budget).** Both apply their rules to a *whole* group at once, so a group must fit the budget at finalize even though cross-group and ingest-time peaks spill. A group larger than `memory.limit` has no in-budget representation, so the run aborts with `E310 MemoryBudgetExceeded` naming the node and the offending `partition_by` group. Raise `memory.limit` clear of the reported figure (finalize also holds the run's other groups, so that figure is a floor, not a target), or drop unread columns in an upstream Transform. **Do not narrow `partition_by` to clear it** — that key defines the group the rules evaluate over, so a narrower key makes the run succeed by changing your results. Cull has a symmetric bound on the *number* of groups: its in-memory drop-decision state is `O(distinct groups)` and cannot spill, so an extreme partition cardinality trips the same code naming the decision state instead.
- **`clinker explain --code E310` is currently Combine-specific.** It describes the Combine case and suggests a `drive:` hint, which most E310 surfaces have no equivalent for ([#1017](https://github.com/rustpunk/clinker/issues/1017)). Until that lands, prefer the per-node guidance linked above; the rendered diagnostic itself always names which surface overran.
- **These are limits, not bugs.** Every case above is an ordinary operational condition with a documented code, so it reports through `E310` rather than as an internal error. The reverse does not hold quite as cleanly: an `internal error` usually means a Clinker defect worth reporting, but a few bounded-memory caps on unusual data shapes still surface that way. If the message names something you control and tells you what to change, follow it; report it as a bug when it does not.
