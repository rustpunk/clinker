...
- **Spillable stages always complete if disk space is available, regardless of input size.** When a blocking stage (sort, hash Aggregate, large Combine) outgrows the budget, it spills to disk instead of failing.
  - **Pure-range (band-join) Combine also spills.** A Combine whose `where:` joins the two inputs only on an inequality (`<`, `<=`, `>`, `>=`) with no equality key — for example a band join such as `orders.amount >= bands.lo and orders.amount < bands.hi` — runs the block-band strategy, which is spill-bounded on both axes: it external-sorts each input side to disk and accumulates its matched output in a spillable sort as well, so it completes on inputs and result sizes larger than the budget. It still aborts with `E310 MemoryBudgetExceeded` only as a last resort — when a single indivisible unit of work (one pair of input blocks plus the join's scratch arrays) cannot fit the hard limit even on its own. If you hit that, give the pipeline more headroom or narrow the predicate.
  - **Exception — equality+range Combine.** A Combine that mixes an equality key with a range predicate runs the hash-partitioned range strategy, which currently holds each partition's state resident (a follow-up bounds it): it enforces the budget on that pre-output working set and aborts with `E310 MemoryBudgetExceeded` if it overruns, rather than spilling. Give such a pipeline enough headroom, or narrow the predicate so fewer rows enter the join.
