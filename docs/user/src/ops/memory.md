...
- **These are limits, not bugs.** Every case above is an ordinary operational condition with a documented code, so it reports through `E310` rather than as an internal error. Three conditions do still present as an `internal error` today despite being about your data or your host, not an engine defect — do not file a bug for these, and see [#1021](https://github.com/rustpunk/clinker/issues/1021), which tracks reclassifying them:
  - a `NaN`, array, or map value in a Reshape or Cull `partition_by` column;
