...
- **A few handoff buffers are memory-only and can trip `E310`.** A stage that fans out to more than one consumer, or that feeds a composition's input port, must hold its rows in memory for those consumers to read (they cannot share a spilled file), so it never spills. Before making the temporary copy needed by an earlier fan-out consumer or a composition input, Clinker adds the copy's estimate to current pressure. If that projected total exceeds the hard limit, it aborts before allocating with `E310 MemoryBudgetExceeded`, names the consumer that needed the copy, and reports `NodeBuffer` as the budget category. This is the one case where a larger input cannot be rescued by more disk — raise `memory.limit`, or restructure so the shared stage feeds a single consumer.
