...
>
> **Rejected range key types (`E327`).** A range conjunct whose operands cannot be reduced to one exact numeric axis is rejected at **plan time** with `E327`, rather than routed to the join where it could silently drop rows or mismatch. Two shapes trigger it: an **ambiguous `numeric` operand** — the result of `abs`, `min`, `max`, or `clamp` over a numeric, whose runtime value may be an integer (compared exactly) or a float — and a **non-orderable pairing** such as a string comparison or a `decimal` compared against a `float`. Compare a concrete matching-typed range key instead. (A supported int/float/decimal/date/datetime range key, including a mixed int/float pair, never triggers `E327`.)
