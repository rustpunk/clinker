...
When `true`, how a carried-along column reaches the writer depends on the output format. Self-describing formats (JSON / NDJSON / XML) write each record's own keys. A CSV output widens its header to the union of every record's columns when it can materialize the batch, and otherwise — on a bounded-memory streaming or envelope-reconstructing path — fails loudly with a `SchemaDrift` error rather than dropping a column it cannot fit under its already-committed header. A fixed-width output has no room for an undeclared column and likewise raises `SchemaDrift`. See [Auto-Widen & Schema Drift → Schema drift across records](../formats/auto-widen.md#schema-drift-across-records-tabular-formats).
