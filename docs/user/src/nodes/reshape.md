...
- **Skew (one giant group):** a single correlation group whose resident tail alone exceeds the budget is spilled **incrementally** — sliced by the upper bits of each record's arrival hash so successive spill waves evict fractions of the one group — while smaller groups stay resident. The *ingest-time* resident peak therefore stays bounded even under one giant skewed group.
