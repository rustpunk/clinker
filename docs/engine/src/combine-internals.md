...
`max_output_rows`, when set on the node, is enforced at the shared output-emit chokepoint: a running emitted-row counter is checked before each push and the run fails loud with `E325` the moment the output would exceed the cap. Because the check sits at the single chokepoint every row passes through, it covers all match modes, the deferred `on_miss` rows, and both the materialized and nested-loop block-pair paths. It is a result-*size* guard, orthogonal to the byte-based spill machinery above (which answers memory pressure by spilling, not by capping rows).
