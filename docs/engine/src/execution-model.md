...
- **`IEJoin` / sort-merge `Combine`** — both inputs are sorted before the band/merge step runs, and both are **block-spilled** so the input axis stays inside the budget, but by different mechanisms. The pure-range IEJoin external-sorts each side to disk and slices the sorted stream into min/max-tagged blocks, pruning non-overlapping block-pairs before the kernel runs; its **output axis is spill-bounded too** — matched rows accumulate in a payload-ordered sort buffer that spills on its own byte threshold (charged through the join's consumer handle) and drains incrementally, streamed straight to a downstream Output or folded into a spillable node-buffer, so both axes are bounded with no global-pressure abort. The sort-merge Combine external-sorts each side into runs and merges matching runs; it has no min/max block tags or pruning. The equi+range IEJoin (hash-partitioned range join) is **buffered resident**: it holds each partition's sorted arrays in memory, and its in-RAM output vec is bounded only by a periodic hard-limit poll (a follow-up bounds both).
