...
4. **Collapse the column to a single value** in a transform before the sink.

5. **Drop `multiple: true`** from the source column if the field never actually
...
6. **If the schema is shared across sources** and only some of them repeat the
