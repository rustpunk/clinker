...
2. **Pull document-level metadata into record fields** through the API's own
   response shape — `record_path` to point at the record array, and
   `split_to_rows` to hoist a nested array's fields onto each record — so the
   value travels as a normal field rather than as document-envelope context.
